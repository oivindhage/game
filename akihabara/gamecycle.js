	...
	return gbox.addObject({
		...
		gameTitleIntroAnimation:function(reset) { 
			if (reset) { // "reset" is true before the first frame of the intro screen. We can prepare the intro animation...
				...
				toys.logos.linear(this,"rising",{image:"logo",x:gbox.getScreenHW()-gbox.getImage("logo").hwidth,y:20,sx:gbox.getScreenHW()-gbox.getImage("logo").hwidth,sy:20,speed:1,audioreach:"intro"}); 
				return true;
			}
		},
		...
		pressStartIntroAnimation:function(reset) {
				...
				gbox.blitFade(gbox.getBufferContext(),{alpha:1});
				//toys.dialogue.render(this,"intro-animation",dialogues.intro);
				return true;
		},
		...
		gameMenu:function(reset) {
			if (reset) {
				toys.resetToy(this,"difficulty");
			 	...
			 	if (toys.ui.menu(this,"difficulty",{audiooption:"default-menu-option",audioconfirm:"default-menu-confirm",font:"small",keys:{up:"up",down:"down",ok:"a"},selector:">",items:["Save the Kittens","Look the other way"],x:80,y:150})) {
			 		if (toys.getToyValue(this,"difficulty","ok") == -1){ 
			 			return 0;
			 		}
			 		else {
			 			
			 			if (toys.getToyValue(this,"difficulty","selected")==0){
			 				return -1;
			 			}else{
			 				gbox.blitFade(gbox.getBufferContext(),{alpha:1});
			 				return -2;
					...
					switch (this.state) {
						...
						case 101: { // Game menu
							...
							if (menu == -1) {
								...
								toys.fullscreen.fadeout(this,"fadeout",gbox.getBufferContext(),{fadespeed:0.05,audiochannelfade:"bgmusic"});
							} 
							break;
						}
						case 102: { // Fader
